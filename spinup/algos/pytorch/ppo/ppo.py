...
def ppo(env, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=2048, epochs=250, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
        vf_lr=1e-3, train_pi_iters=100, train_v_iters=70, lam=0.95, max_ep_len=512,
        target_kl=0.005, logger_kwargs=dict(), save_freq=5):
    """
    ...
    def compute_loss_pi(data):
        ...
        # Policy loss
        pi, logp = ac.pi(obs, act)#变化的是网络pi   data['obs']，data['act']，data['adv']，data['logp']在回合内未变
    ...
    for epoch in range(epochs):
        ...
        writer.add_scalar('AverageEpRet', stats_to_write[0], global_step = (epoch + 1) * 2048)

        # Log info about epoch  一个回合的数据
        logger.log_tabular('Epoch', epoch)                                  #第几个回合
        logger.log_tabular('EpRet', with_min_and_max=True)                  #回报的最大、最小、平均值，游戏结束时停留的状态的回报
        logger.log_tabular('EpLen', average_only=True)                      #单次游戏长度的平均值
        logger.log_tabular('VVals', with_min_and_max=True)                  #值函数的最大、最小、平均值
        logger.log_tabular('TotalEnvInteracts', (epoch+1)*steps_per_epoch)  #目前总步数
