        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            with self.sess.as_default():
                ...
                while True:
                    ...
                    if len(len_buffer) > 0:
                        logger.record_tabular("EpLenMean", np.mean(len_buffer))#每次运行的步数（平均）
                        logger.record_tabular("EpRewMean", np.mean(reward_buffer))#每次运行的最后回报（平均）
                    logger.record_tabular("EpThisIter", len(lens))#每回合的运行次数
