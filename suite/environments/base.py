...
from suite.utils import SimulationError, XMLError, MujocoPyRenderer
...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def initialize_time(self, control_freq):
        ...
        """
        self.cur_time = 0
        self.model_timestep = self.sim.model.opt.timestep#0.002s
    ...
    def step(self, action):
        ...
        for i in range(steps):
            ...
            plot_list.append(self._right_hand_pos)#获取需要plot的信息
            self._pre_action(action, policy_step)
            self.sim.step()
            policy_step = False
            if self.has_renderer == True:
                if(i%5 == 0):
                    self.render()
        ...
        return self._get_observation(), reward, done, info, plot_list
    ...
    def _post_action(self, action):
        ...
        # 水平范围超出就停止
        x_out = (self.ep_pos[0] > self.us_pos[0] + 0.2) or (self.ep_pos[0] < self.us_pos[0] - 0.2)
        y_out = (self.ep_pos[1] > self.us_pos[1] + 0.2) or (self.ep_pos[1] < self.us_pos[1] - 0.2)
        self.done = (x_out or y_out)
