...
from suite.environments import MujocoEnv

from suite.models.grippers import gripper_factory
from suite.models.robots import Panda

from suite.controllers.arm_controller import *
...
class PandaEnv(MujocoEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.goal = np.zeros(3)#目标位置
        self.goal_orientation = np.zeros(3)#目标方向
        self.desired_force = np.zeros(3)#期望力
        self.desired_torque = np.zeros(3)#期望力矩
        ...
        self.contact_threshold = 1  # Maximum contact variation allowed without contact [N] 阈值
    ...
    def _load_controller(self, controller_type, controller_file, kwargs):# 加载控制器
        """
    ...
    def _pre_action(self, action, policy_step):
        ...
        gripper_action = []
        if self.has_gripper:
            gripper_action = action[self.controller.control_dim:]  # all indexes past controller dimension indexes   手爪动作：1维
            action = action[:self.controller.control_dim]   #关节动作：7维

        # TODO
        # First, get joint space action
        # action = action.copy()  # ensure that we don't change the action outside of this scope
        self.controller.update_model(self.sim, id_name='right_hand', joint_index=self._ref_joint_pos_indexes)# 获得当前的运动信息，包括雅克比矩阵
        torques = self.controller.action_to_torques(action,
                                                    self.policy_step)  # this scales and clips the actions correctly 由以上信息和action计算出当前需要的力矩大小
        ...
        # Now, control both gripper and joints 控制运动
        self.sim.data.ctrl[self._ref_joint_vel_indexes] = self.sim.data.qfrc_bias[
                                                                self._ref_joint_vel_indexes] + torques

        if self.policy_step:
            self.prev_pstep_q = np.array(self.curr_pstep_q)#记录上一步位置7
            self.curr_pstep_q = np.array(self.sim.data.qpos[self._ref_joint_vel_indexes])#记录当前步位置7
            self.prev_pstep_a = np.array(self.curr_pstep_a)#动作
            self.curr_pstep_a = np.array(action)  # .copy()) # TODO
            self.prev_pstep_t = np.array(self.curr_pstep_t)#控制力矩7
            self.curr_pstep_t = np.array(self.sim.data.ctrl[self._ref_joint_vel_indexes])
            self.prev_pstep_ft = np.array(self.curr_pstep_ft)#末端力
            ...
            self.prev_pstep_ee_v = self.curr_pstep_ee_v#末端执行器速度
            ...
            self.buffer_pstep_ee_v.popleft()#保存十个末端执行器速度
            ...
            diffs = np.diff(buffer_mat, axis=0)#v(n+1)-v(n)
            diffs *= self.control_freq#dv/0.01
    ...
    def _post_action(self, action):
        ...
        self.ee_pos = np.array(self.sim.data.body_xpos[self.sim.model.body_name2id('right_hand')])#获取末端位置

        force_sensor_id = self.sim.model.sensor_name2id("force_ee")
        self.ee_force = np.array(self.sim.data.sensordata[force_sensor_id * 3: force_sensor_id * 3 + 3])#获取末端力

        if np.linalg.norm(self.ee_force_bias) == 0:#bias合力为零
            self.ee_force_bias = self.ee_force

        torque_sensor_id = self.sim.model.sensor_name2id("torque_ee")
        self.ee_torque = np.array(self.sim.data.sensordata[torque_sensor_id * 3: torque_sensor_id * 3 + 3])#获取末端力矩

        if np.linalg.norm(self.ee_torque_bias) == 0:#bias合力矩为零
    ...
    def _get_observation(self):
        ...
        #获取末端力
        force_sensor_id = self.sim.model.sensor_name2id("force_ee")
        di["ee_force"] = np.array(self.sim.data.sensordata[force_sensor_id * 3: force_sensor_id * 3 + 3])#获取末端力
        # print(di["ee_force"])

        #获取末端力矩
        torque_sensor_id = self.sim.model.sensor_name2id("torque_ee")
        di["ee_torque"] = np.array(self.sim.data.sensordata[torque_sensor_id * 3: torque_sensor_id * 3 + 3])#获取末端力矩

        #洞口到末端的矢量
        _hole_pos = self.sim.data.site_xpos[self.sim.model.site_name2id("underside")] + [0, 0, 0.06]
        _ep_pos = self.sim.data.site_xpos[self.sim.model.site_name2id("endpoint")]
        di["delta_pos"] =  _ep_pos - _hole_pos
        return di
